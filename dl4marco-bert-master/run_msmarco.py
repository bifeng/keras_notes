...
def input_fn_builder(dataset_path, seq_length, is_training,
  ...
  def input_fn(params):
    ...
    def extract_fn(data_record):
      ...
      As the convert_single_example function in the original google bert code.
    ...
    '''
    '''
    # todo buffer_size of shuffle is correlate with number of docs per query (num_eval_docs) ?
    # todo How to shuffle the dataset for rank? (Inter-group and intra-group for each query group)
    if is_training:
  ...
  if FLAGS.do_eval:
    for set_name in ["dev", "eval"]:
      ...
      for item in result:
        ...
        '''
        calculate metric for each query.
        '''
