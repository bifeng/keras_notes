...
def write_to_tf_record(writer, tokenizer, query, docs, labels,
  ...
  '''
  :param writer:
  :param tokenizer: word segmentation etc.
  :param query: one query
  :param docs: multi docs
  :param labels: multi labels
  :param ids_file:
  :param query_id:
  :param doc_ids:
  :return:
  ...
  doc   -> doc token ids     ->  tf.train.Feature  ->  tf.train.Example
  label -> label
...
def convert_eval_dataset(set_name, tokenizer):
  ...
  '''
  For dev sets, construct queries_docs dict {query: [(doc_id, doc, label),...]}
  '''
  if set_name == 'dev':
